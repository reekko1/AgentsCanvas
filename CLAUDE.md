...
Toolbar **New Agent** / **New Diff** = create (folder picker) · N / ⌘N = new card · double-click item = frame it (a card spawns if dormant; a diff just flies — diffs are live, never lazy; flying into a card releases its held permission asks to the terminal dialog) · Tab = fly to the oldest card that needs you · double-click empty / esc / space / ⌘0 = fit all · click/type in a card = interact with its terminal · +/− = zoom · ✕ on title bar = delete (card → kills its tmux session; diff → stop watcher). All single-knob tunables (magnifications, sizes incl. `diffSize`) live in `Viewport` / `CanvasLayout`; the stall threshold lives in `CanvasViewController.stallAfter`.
