...
Agent Canvas — a native macOS infinite-canvas app for supervising many coding agents at once ("telescope, not cockpit"; **observe, don't orchestrate** *agents*). The card god-view is read-only: the only verb is *click a card → fly in*. **Scoped exception:** the Diff Object permits explicit, user-initiated git actions (stage/unstage/discard/commit) on its working tree, with destructive ones confirmed — direct working-tree manipulation, not agent orchestration. (The original PRD/design-brief docs were retired; this file is the product spec of record. "PRD §" citations in code comments refer to that retired doc — treat them as "deliberate product decision", not as a pointer.)
...
- **Onboarding/** — the first-run wizard (implemented from the Claude Design "Beads" handoff): `OnboardingDialog.swift` (scrim overlay + the 432px fixed-frame dialog, header progress beads in the status-bead language, the step state machine — claude/tmux **self-complete** ~650ms after the probe finds them, satisfied steps are skipped, tailscale waits at its QR reward for Continue), `OnboardingSteps.swift` (step content + flag pills, "watching for it…" rows, QR reward via `CIQRCodeGenerator`, the phone-peek illustration, `WizardButton`). Presented by the controller once ever (`UserDefaults` "onboardingCompleted") on an empty canvas; every dismissal (×, scrim, Esc, "Skip setup") lands on the empty state's readiness rows. Severity decides the skips: claude gates (none), tmux warns (visible + cost line), tailscale invites (prominent). "Set Up Remote Access…" in the app menu reopens just the tailscale chapter (`mode: .remoteOnly`). Probes ride `Support/Readiness.swift` (claude via login shell — exactly how a card spawns; tailscale serve status checked against the live remote-panel port, read lazily because the server binds async).
