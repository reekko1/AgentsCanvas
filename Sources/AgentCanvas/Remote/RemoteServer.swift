...
/// The JSON projection of the canvas's attention state — what the remote panel
/// shows. The controller publishes a fresh snapshot through the same funnel that
/// refreshes the in-app activity center, so the two can never disagree.
struct RemoteState: Codable {
    struct Card: Codable {
        let id: String
        let name: String
        let status: String          // CardStatus.word
        let loud: Bool
        let since: Double           // epoch seconds the status began
        let task: String?
        let model: String?
    ...
    struct Approval: Codable {
        let id: String              // PermissionAsk UUID
        let name: String
        let detail: String
        let created: Double
    }
...
/// The remote panel: supervision — and the orbit Allow/Deny — from any device on
/// your tailnet. `GET /` serves a self-contained page, `GET /state` the JSON
/// snapshot, `POST /decide {id, allow}` answers a held permission ask.
///
/// It binds loopback only; reachability is deliberately a proxy's job:
/// `tailscale serve --bg localhost:<port>` adds TLS + tailnet identity. **Never
/// expose it publicly** (Funnel, port-forward): the Allow button approves
/// arbitrary tool calls on this machine.
final class RemoteServer {
    /// A decision arriving from the panel (main thread) — same authority as the
    /// in-app activity center's buttons, routed to the same `decideAsk`.
    ...
    /// Main thread only — the same thread the request handler reads on.
    func publish(_ state: RemoteState) {
        stateJSON = (try? JSONEncoder().encode(state)) ?? Data("{}".utf8)
    }

    private func handle(_ request: HTTPServer.Request, _ respond: @escaping (HTTPServer.Response) -> Void) {
        switch (request.method, request.path) {
        case ("GET", "/"):
            respond(.html(Self.page))
        case ("GET", "/state"):
            respond(.json(stateJSON))
        case ("POST", "/decide"):
            guard let obj = try? JSONSerialization.jsonObject(with: request.body) as? [String: Any],
                  ...
                  let allow = obj["allow"] as? Bool else {
                respond(.badRequest)
                return
            }
            onDecide?(id, allow)
    ...
    /// Colors mirror Theme's dark "lo-fi dusk" palette by hand — keep in sync.
    ...
    <style>
      :root { --bg:#13111c; --body:#201f2a; --bar:#2a2836; --text:#eee9df; --muted:#92909f;
              --border:#393744; --blocked:#fbb636; --error:#f33f4d; --done:#76ce99; }
      * { box-sizing: border-box; margin: 0; }
      ...
      .tile.err { border-color: var(--error); box-shadow: 0 0 18px rgba(243,63,77,.2); }
    ...
    <script>
    var COLORS = { idle:'#807e90', running:'#48c0c1', waiting:'#92aae4', done:'#76ce99',
                   stalled:'#d79a56', blocked:'#fbb636', error:'#f33f4d' };
